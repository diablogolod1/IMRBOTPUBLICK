...
# 🔢 ВЕРСИЯ ПРОГРАММЫ
# ============================================
VERSION = "0.01"  # ✅ Текущая версия (увеличивается на 0.01 при каждом обновлении)

# ============================================
# 🌐 ПРОВЕРКА ОБНОВЛЕНИЙ
# ============================================
class UpdateChecker:
    # ✅ Ссылка на актуальный код (Google Drive)
    UPDATE_URL = "https://drive.google.com/uc?export=download&id=1p0K10cfaPNVLFlj_ix1fgu64JuzFY-pT"
    VERSION_FILE = Path("macros/version.json")
    ...
    def get_remote_version():
        """✅ Получение версии с сервера (читаем только первую строку с версией)"""
        try:
            # Скачиваем только первые 200 байт для получения версии
            req = urllib.request.Request(UpdateChecker.UPDATE_URL, headers={'Range': 'bytes=0-200'})
            with urllib.request.urlopen(req, timeout=10) as response:
                content = response.read().decode('utf-8', errors='ignore')
                # Ищем строку VERSION = "x.xx"
                import re
    ...
    def compare_versions(local, remote):
        """✅ Сравнение версий: возвращает True если есть обновление"""
        if not local or not remote:
            return False
        try:
            local_parts = [float(x) for x in local.split('.')]
            remote_parts = [float(x) for x in remote.split('.')]
    ...
    def download_update(callback=None):
        ...
        try:
            temp_file = Path("IMRBOT_update.tmp")
            ...
            if temp_file.exists() and temp_file.stat().st_size > 1000:  # Минимальная проверка
    ...
    @staticmethod
    def apply_update(new_file, main_file="IMRBOT.py"):
        """✅ Применение обновления: замена файла"""
        try:
            # Создаём бэкап
            backup = Path(f"{main_file}.backup")
            if Path(main_file).exists():
                shutil.copy2(main_file, backup)
            
            # Заменяем файл
            shutil.move(new_file, main_file)
            
            # Удаляем бэкап если всё успешно
            if Path(main_file).exists() and backup.exists():
                backup.unlink()
            ...
            backup = Path(f"{main_file}.backup")
            if backup.exists() and not Path(main_file).exists():
                shutil.copy2(backup, main_file)
            return False

# Отключение масштабирования Windows для точности пикселей
try:
...
# Попытка импорта OCR библиотеки
OCR_AVAILABLE = False
try:
    import pytesseract
...
class MacroRecorder:
        ...
        try:
            ...
            while not self.stop_thread:
                try:
                    ...
                    print(f"Ошибка в mouse listener: {e}")
                    time.sleep(0.1)
        except ImportError:
    ...
    def record_click(self, x, y):
        try:
            ...
            print(f"📝 Записан клик #{len(self.events)}: ({rx}, {ry}) через {time_diff:.2f} сек")
        except Exception as e:
    ...
    def save_macro(self, filename=None, custom_path=None):
        try:
            ...
            print(f"✅ Макрос сохранён: {filepath.absolute()}")
...
class ServerSelectorRecorder:
    ...
    def start_recording(self, hwnd):
        try:
            ...
            self.mouse_thread = threading.Thread(target=self._mouse_listener_loop, daemon=True)
            self.mouse_thread.start()
            print(f"🔴 ЗАПИСЬ ВЫБОРА СЕРВЕРА НАЧАТА (окно: {hwnd})")
        except Exception as e:
    ...
    def _mouse_listener_loop(self):
        try:
            ...
            print(f"Критическая ошибка в записи: {e}")
    ...
    def stop_recording(self):
        try:
            ...
            if self.mouse_thread and self.mouse_thread.is_alive():
                self.mouse_thread.join(timeout=2.0)
            print(f"⏹️ ЗАПИСЬ ВЫБОРА СЕРВЕРА ОСТАНОВЛЕНА (записано событий: {len(self.events)})")
        except Exception as e:
            print(f"Ошибка остановки записи: {e}")
    ...
    def save_server_macro(self, filepath):
        try:
            ...
            down_count = sum(1 for e in self.events if e['type'] == 'MOUSE_DOWN')
            drag_count = sum(1 for e in self.events if e['type'] == 'MOUSE_DRAG')
            up_count = sum(1 for e in self.events if e['type'] == 'MOUSE_UP')
            
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write("# Макрос выбора сервера (ЗАЖАТИЕ + ПЕРЕТАСКИВАНИЕ)\n")
                f.write(f"# Всего: {len(self.events)} (нажатий: {down_count}, перетаскиваний: {drag_count}, отпусканий: {up_count})\n\n")
                for event in self.events:
                    f.write(f"{event['type']}:{event['x']},{event['y']},{event['time']:.3f}\n")
            
            print(f"✅ Макрос сохранён: {filepath}")
            return filepath
        except Exception as e:
...
class BotWorker(QObject):
    ...
    def toggle_pause(self):
        try:
            ...
            if self.paused:
                ...
                self.log_msg.emit("⏸️ ПАУЗА (нажмите F5 для продолжения)")
            else:
                self.pause_event.set()
                self.log_msg.emit("▶️ ПРОДОЛЖЕНИЕ выполнения")
        except Exception as e:
            self.log_msg.emit(f"❌ Ошибка паузы: {e}")
    ...
    def find_game_hwnd(self):
        try:
            ...
            if hwnds:
                ...
                self.log_msg.emit(f"✓ Найдено окно: {title}")
                return hwnds[0]
            else:
                self.log_msg.emit("✗ Окно игры НЕ найдено!")
                return None
        except Exception as e:
            self.log_msg.emit(f"❌ Ошибка поиска окна: {e}")
            return None
    ...
    def get_window_region(self, hwnd):
        try:
            ...
            self.log_msg.emit(f"❌ Ошибка получения региона окна: {e}")
    ...
    def absolute_click(self, hwnd, rx, ry, delay=0.5):
        try:
            ...
            self.log_msg.emit(f"→ КЛИК: ({rx:.2f}, {ry:.2f}) в ({tx}, {ty})")
            return True
        except Exception as e:
            self.log_msg.emit(f"✗ Ошибка клика: {e}")
            return False
    ...
    def move_cursor_to(self, hwnd, rx, ry, delay=0.3):
        try:
            ...
            self.log_msg.emit(f"→ КУРСОР: ({rx:.2f}, {ry:.2f}) в ({tx}, {ty}) [без клика]")
            return True
        except Exception as e:
            self.log_msg.emit(f"✗ Ошибка перемещения курсора: {e}")
            return False
    ...
    def execute_server_macro(self, hwnd):
        try:
            ...
            self.log_msg.emit("\n=== ВЫПОЛНЕНИЕ ВЫБОРА СЕРВЕРА ===")
            
            events = self._load_server_macro(self.server_macro_path)
            if not events:
                self.log_msg.emit("Не удалось загрузить события макроса")
                return
            
            down_count = drag_count = up_count = 0
            ...
            self.log_msg.emit(f"Выбор сервера завершён (нажатий: {down_count}, перетаскиваний: {drag_count}, отпусканий: {up_count})")
        except Exception as e:
            self.log_msg.emit(f"❌ Ошибка выполнения макроса сервера: {e}")
    ...
    def run_macro_only(self, hwnd):
        try:
            self.log_msg.emit("\n=== ЗАПУСК ТОЛЬКО МАКРОСА (БЕЗ АВТОРИЗАЦИИ) ===")
            ...
            if self.active_macro:
                self.log_msg.emit(f"📜 Запуск макроса...")
        ...
        try:
            ...
            if command.startswith('TEXT:'):
                ...
                if text_to_type == '{EMAIL}':
                    ...
                    self.log_msg.emit(f"⌨️ Ввод EMAIL аккаунта")
                elif text_to_type == '{PASSWORD}':
                    text_to_type = self.current_password
                    self.log_msg.emit(f"⌨️ Ввод PASSWORD аккаунта")
                else:
...
class InfiniteBotApp(QMainWindow):
    ...
    def register_hotkeys(self):
        try:
            ...
            self.log_msg("✅ Горячие клавиши активированы (F1-F5)")
        except Exception as e:
            self.log_msg(f"❌ Ошибка регистрации горячих клавиш: {e}")
    ...
    def _hotkey_listener(self):
        try:
            ...
            print(f"❌ Ошибка в hotkey listener: {e}")
    ...
    def select_macro_folder(self):
        try:
            folder = QFileDialog.getExistingDirectory(
                self,
                "Выберите папку для макросов",
                str(self.macro_folder_path)
            )
    ...
    def select_image_folder(self):
        try:
            folder = QFileDialog.getExistingDirectory(
                ...
                str(self.image_finder.get_image_folder())
            )
        ...
        try:
            if state == Qt.CheckState.Checked:
                if not self.server_recorder.is_recorded() and not self.server_macro_path.exists():
                    QMessageBox.warning(
                        self, "Ошибка",
                        "Сначала выполните запись выбора сервера!\n\n"
                        "1. Нажмите 'Запись выбора сервера'\n"
                        "2. Зажмите ЛКМ и перетащите курсор\n"
                        "3. Отпустите ЛКМ\n"
                        "4. Нажмите F2 для остановки\n"
                        "5. Затем отметьте галочку"
                    )
                    self.chk_use_server.setChecked(False)
                    return
    ...
    def delete_current_macro(self):
        ...
        try:
            ...
            reply = QMessageBox.question(
                ...
                f"Вы действительно хотите удалить макрос '{name}'?\n\nЭто действие нельзя отменить!",
            ...
            if self.config_path.exists():
                ...
                if accounts_path and os.path.exists(accounts_path):
                    ...
                    try:
                        ...
                        self.append_to_log(f"Загружен файл аккаунтов: {accounts_path}")
                    except Exception:
                        pass
        ...
        try:
            ...
            if not templates:
                ...
                QMessageBox.information(
                    self, "Нет шаблонов",
                    f"Папка {folder} пуста или не существует!\n\n"
                    "ИНСТРУКЦИЯ:\n"
                    "1. Положите картинки (.png/.jpg) в папку изображений\n"
                    "2. Нажмите '📂 Выбрать папку' для смены папки\n"
                    "3. Название файла = название команды (без расширения)\n"
                    "4. Нажмите эту кнопку снова для выбора"
                )
                return
    ...
    def add_img_command(self):
        try:
            ...
            dialog.setLabelText(f"Выберите изображение\n(папка: {self.image_finder.get_folder_path()})\n(найдено: {len(templates)})")
